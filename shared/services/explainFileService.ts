 ...
 *
 * @example
 * const analysis = normalizeFileAnalysis({ summary: "Reads config", detail: "..." });
 */
function normalizeFileAnalysis(value: unknown): FileAnalysis {
  ...
  if (!summary || !detail) {
    throw new Error("Invalid file analysis response: summary and detail are required.");
  }
 ...
 *
 * @example
 * const summary = nonEmptyString("Reads config");
 */
