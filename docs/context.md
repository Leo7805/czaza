...
- Shared Runtime State detection entry points are owned by `RuntimeNoteStateDetectionController`: current-file detection reconciles all Note levels, while project File Note detection selects indexed File Note resources and preserves complete per-resource Registry state.
